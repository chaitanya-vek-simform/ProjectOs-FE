...
export const getErrorMessage = (error: Error, fallback: string): string => {
    const axiosError = error as AxiosError<{ message?: string | Array<{ field: string; message: string }> }>;
 ...
 */
export function exportToCSV<T extends Record<string, unknown>>(data: T[], filename: string, headers?: Record<string, string>): void {
    if (data.length === 0) return;

    const keys = Object.keys(data[0]);
    const headerRow = headers ? keys.map((key) => headers[key] || key).join(',') : keys.join(',');
