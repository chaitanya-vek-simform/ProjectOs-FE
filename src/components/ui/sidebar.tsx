...
function Sidebar({
    ...
    if (collapsible === 'none') {
        return (
            <div
                data-slot="sidebar"
                className={cn('flex h-full w-(--sidebar-width) flex-col bg-sidebar text-sidebar-foreground rounded-lg', className)}
                {...props}
            >
                {children}
            </div>
...
function SidebarMenu({ className, ...props }: React.ComponentProps<'ul'>) {
    return (
        <ul
            ...
            data-sidebar="menu"
            className={cn('flex w-full min-w-0 flex-col gap-1 m-0 list-none', className)}
            {...props}
        />
    );
}
